  ...
  return async (dispatch) => {
    ...
    try {
      const response = await fetch(
        `https://dataservice.accuweather.com/locations/v1/cities/autocomplete?apikey=tifBSVtKNWx7qdSGbrZsi6usHmoijA2q&q=${input}`
      );
...
export const selectCity = (city) => {
  ...
  return async (dispatch) => {
    ...
    try {
      const response = await fetch(
        `https://dataservice.accuweather.com/currentconditions/v1/${city.Key}?apikey=tifBSVtKNWx7qdSGbrZsi6usHmoijA2q&details=true`
      );
...
export const displayFiveDayInfo = (cityKey) => {
  ...
  return async (dispatch) => {
    ...
    try {
      const response = await fetch(
        `https://dataservice.accuweather.com/forecasts/v1/daily/5day/${cityKey}?apikey=tifBSVtKNWx7qdSGbrZsi6usHmoijA2q&metric=true`
      );
...
export const fetchGeolocationCity = (lat, lng) => {
  return async (dispatch) => {
    ...
    try {
      const response = await fetch(
        `https://dataservice.accuweather.com/locations/v1/cities/geoposition/search?apikey=%09tifBSVtKNWx7qdSGbrZsi6usHmoijA2q&q=${lat}%2C${lng}`
      );
